...
public class JwtServiceImpl implements JwtService {
    ...
    private static final String USERNAME_CLAIM = "username", ROLE_CLAIM = "role", LIBRARY_CLAIM = "library";
    ...
    @Override
    public String generateToken(LoginAdminDto admin) throws JOSEException {
        ...
        JWTClaimsSet claims = new JWTClaimsSet.Builder(PUBLIC_CLAIMS_SET).issueTime(issueTime)
                .expirationTime(expirationTime).claim(USERNAME_CLAIM, admin.getUsername())
                .claim(ROLE_CLAIM, admin.getRole()).claim(LIBRARY_CLAIM, admin.getLibrary()).build();
    ...
    @Override
    public SignedJWT convert(@NotNull String token) throws ParseException {
    ...
    @Override
    public LoginAdminDto verify(SignedJWT token) throws JOSEException, ParseException {
        ...
        LoginAdminDto admin = new LoginAdminDto((String) claims.getClaim(USERNAME_CLAIM),
                Role.valueOf((String) claims.getClaim(ROLE_CLAIM)),
                Library.valueOf((String) claims.getClaim(LIBRARY_CLAIM)));
        return admin;
    }
