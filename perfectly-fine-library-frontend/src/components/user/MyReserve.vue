<template>
    <div id="reserve_container">
        <h3>我的预约</h3>
        <div v-if="reNumber > 0" style="margin-top: -100px">
            <el-row :gutter="10" v-for="n in getRowCount" :key="n" class="row">
                <template v-for="item in books.slice((currentPage - 1)*(width * height) + (n - 1)*width,
                                            (currentPage - 1)*(width  *height) + n * width)" :key="item.bookId"
                                            style="height: 300px">
                    <el-col :span="6" style="padding: 100px;height: 300px">
                        <el-tooltip placement="right" class="el-tooltip" style="height: 250px">
                            <template #content>
                                <p>{{item.title}}</p>
                                <p>
                                    {{item.author}}
                            ...
                            <el-card bodyStyle="padding:10px" shadow="hover" class="book"
                                     style="width: 220px;height: 233px;"
                                        @click="toDetail(item.bookId)">
                                <div class="cover">
                                    <img style="height: 200px" :src="item.cover" alt="封面">
                                </div>
                                <div class="reserveInfo" style="height: 45px">


                                    <p class="p-time">所在分馆：{{item.library}}</p>

                                    <p class="p-time">预约时间：{{item.expiration}}</p>
                                </div>
                            </el-card>
                        </el-tooltip>
                    </el-col>
            ...
            myReserve(){
              getUserInfo()
                  .then(resp => {
                      switch (resp.status) {
                          case 200:
                              this.reserve = resp.data.reservedCopyIds;
                              if (this.reserve.length > 0){
                                  this.reNumber = this.reserve.length;
                                  this.totalPage = Math.floor((this.reNumber - 1)/(this.width * this.height)) + 1;
                                  this.totalRow = Math.floor((this.reNumber - 1)/(this.width)) + 1;
                                  this.books.length = this.reserve.length;

                                  for (let i = 0; i < this.reserve.length; i++){
                                      getCopyFromCopyId(this.reserve[i])
                                          .then(resp => {
                                          if (resp.status === 200) {
                                              ...
                                              getBookFromBookId(this.bookId)
                                                  .then(resp => {
                                                      if (resp.status === 200) {
                                                          ...
                                                          this.books[i].library=aaa.library;
                                                          let a=new Date(aaa.expiration);
                                                          ...
                                                          this.books[i].expiration=year+"-"+month+"-"+day+s+hour+t+minute+u+second;
                                                         // this.books[i].expiration=a;
                                                      //    alert( this.books[i].expiration+8*60*60*1000)
                                                      } else {
                                                          // alert('错误');
                                                      }
                                                  })
        ...
        computed:{
            getPageBookCount() {// 返回页面该展示的书籍的本数
                if (this.currentPage === this.totalPage){// 如果当前页面是最后一页
                    return (this.reNumber % (this.width * this.height));
